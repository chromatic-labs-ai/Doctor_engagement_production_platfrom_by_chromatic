...
async function SupervisorRequestsTable({
  companyId,
  query,
  status,
  operatorFilter,
      ...
      <div className="flex flex-col gap-6 md:gap-8">
        <div className="flex flex-col gap-4 border-b pb-5 sm:flex-row sm:items-end sm:justify-between">
          <div>
            ...
            <h2 className="mt-2 text-3xl font-semibold tracking-[-0.02em]">Supervisor Dashboard</h2>
        ...
        <Card>
          <CardHeader className="gap-4 pb-0">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Monitor requests by operator and status without changing workflow data.
              </p>
            </div>
            <DashboardFilterForm
              className="grid gap-3 md:grid-cols-[minmax(0,1fr)_220px_220px_auto] md:items-center"
            ...
            >
              ...
              <div className="relative">
                <UsersIcon className="absolute left-3 top-3.5 size-4 text-muted-foreground" />
                <select
                  name="operator"
          ...
          <Suspense
            key={`${query}:${operatorFilter}:${status}`}
            fallback={
